...
from path import evaluate_path_prediction
    ...
    for img_id, img_info in enumerate(gt_data['images']):
        ...
        matched_pred_img = next(
            (pred_img for pred_img in pred_data['images']
             if pred_img.get('id') == gt_image_id and pred_img.get('file_name') == gt_file_name),
            None
        )
...
def generate_final_leaderboard(submissions_root: str,
    ...
    try:
        ...
        df['norm_rmse'] = normalize(df['rmse'], reverse=True)
